...
GuardPR は、セキュリティ脆弱性の検出、修正パッチの生成、テストによる検証、Draft PR の作成までを 1 回のワークフロー実行で自動化する GitHub Action です。Community モードではコードや検出結果は外部送信されません。Pro は明示的オプトイン時のみ統計情報を送信します。
...
| 認可スキャン | Yes | Yes |
| 自動修正 PR | Yes | Yes |
| 監査ログアーティファクト | Yes | Yes |
| ダッシュボード & 分析 | -- | 予定 |
| カスタム検出ルール | -- | 予定 |
| Slack/Teams 通知 | -- | 予定 |
| 優先サポート | -- | 予定 |

Pro 連携は `pro-api-key` アクション入力によるオプトインです。Community ユーザーには影響なく、ランナー外にデータが送信されることはありません。プライバシー設計の詳細は [ADR-009](docs/adr/009-pro-opt-in-telemetry.md) を参照してください。
...
| `pro-api-key` | いいえ | -- | Pro ダッシュボード連携用 API キー（GitHub Secrets 推奨） |
...
すべての処理は GitHub Actions ランナー上で実行されます。Community モードではコードや検出結果は外部送信されません。Pro モードでは `pro-api-key` 設定時のみ統計情報が送信されます。パイプラインの詳細は [docs/architecture.md](docs/architecture.md) を参照してください。
