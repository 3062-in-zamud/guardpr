...
import { buildTeamsCard, sendTeamsNotification } from "../../../src/notifications/teams";
...
describe("buildTeamsCard", () => {
  it("uses attention style when P0 > 0", () => {
    const payload = buildTeamsCard(makeCtx({ bySeverity: { P0: 1, P1: 0, P2: 0 } })) as any;
    expect(payload.attachments[0].content.style).toBe("attention");
  });

  it("uses warning style when P1 only", () => {
    const payload = buildTeamsCard(makeCtx({ bySeverity: { P0: 0, P1: 2, P2: 0 } })) as any;
    expect(payload.attachments[0].content.style).toBe("warning");
  });

  it("uses good style when no P0/P1", () => {
    const payload = buildTeamsCard(makeCtx({ bySeverity: { P0: 0, P1: 0, P2: 1 } })) as any;
    expect(payload.attachments[0].content.style).toBe("good");
  });

  it("includes finding counts", () => {
    const payload = buildTeamsCard(makeCtx()) as any;
  ...
  it("excludes PR text when prUrl is undefined", () => {
    const payload = buildTeamsCard(makeCtx()) as any;
    const text = JSON.stringify(payload);
  ...
  it("includes repository name", () => {
    const payload = buildTeamsCard(makeCtx({ repository: "my-org/my-repo" })) as any;
    const text = JSON.stringify(payload);
    expect(text).toContain("my-org/my-repo");
  });

  it("generates card even when all counts are 0", () => {
    const payload = buildTeamsCard(
      makeCtx({ highConfidenceCount: 0, lowConfidenceCount: 0, bySeverity: { P0: 0, P1: 0, P2: 0 } }),
    ) as any;
    expect(payload.attachments[0].content.body.length).toBeGreaterThan(0);
  });
});
...
describe("sendTeamsNotification", () => {
  ...
  it("sends successfully on 200", async () => {
    await sendTeamsNotification(`http://localhost:${serverPort}/webhook`, makeCtx());
  ...
  it("throws on non-2xx after retry", async () => {
    ...
    await expect(
      sendTeamsNotification(`http://localhost:${serverPort}/webhook`, makeCtx()),
